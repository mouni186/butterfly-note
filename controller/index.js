...
const readAllButterflyRemainder = async (req, res) => {
    ...
    try {
        ...
        const result = await controllerRepo.readAllRemainder(requestedData);
        res.status(200).send(result);
    }
    catch (error) {
        res.status(500).send(error);
...
const deleteButterflyRemainder = async (req, res) => {
    ...
    try {
        ...
        const result = await controllerRepo.deleteRemainder(requestedData);
        res.status(200).send(result);
    }
    catch (error) {
        res.status(500).send(error);
...
module.exports = {
    ...
    butterflySignup,
    butterflyNote,
    readAllButterflyRemainder,
    deleteButterflyRemainder
}